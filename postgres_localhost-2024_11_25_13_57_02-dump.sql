...
ALTER TABLE public.roomtypes ALTER COLUMN roomtypes_id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.roomtypes_roomtypes_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
COPY public.hotels (hotels_id, hotel_name, phonenumber, email, starsreview, gym, elevator, distancetobeach, distancetocentrum, pool, restaurant, childrensclub, eveningentertainment, city, country, zipcode, adress, price) FROM stdin;
\.
...
SELECT pg_catalog.setval('public.clients_id_seq1', 2, true);
...
SELECT pg_catalog.setval('public.hotels_hotels_id_seq', 1, false);
...
SELECT pg_catalog.setval('public.rooms_rooms_id_seq', 1, false);
...
SELECT pg_catalog.setval('public.roomtypes_roomtypes_id_seq', 4, true);
